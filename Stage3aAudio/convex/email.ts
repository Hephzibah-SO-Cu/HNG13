...
// --- FIX: Define ALL args that `orders.ts` is sending ---
...
export const sendConfirmation = action({
  args: {
    ...
    orderId: v.id("orders"), // The new ID from the mutation
  },
  // --- END FIX ---
    ...
    const emailHtml = `
      <html>
        <body style="font-family: Arial, sans-serif; padding: 20px;">
          ...
          <p>Your order (ID: <strong>${args.orderId}</strong>) has been received.</p>
